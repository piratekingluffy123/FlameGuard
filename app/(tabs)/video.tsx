...
export default function App() {
  ...
  // ✨ MODIFIED: Fetch videos with pagination
  const fetchVideos = async (loadMore = false) => {
    try {
      ...
      let url = `https://api.cloudinary.com/v1_1/${cloudName}/resources/video?max_results=10`;
      ...
      if (loadMore) {
        ...
        setVideos(prevVideos => [...prevVideos, ...res.data.resources]);
      } else {
      ...
      console.log(`✅ Fetched ${res.data.resources.length} videos from Cloudinary`);
      if (res.data.next_cursor) {
        console.log(`📄 More videos available, next cursor: ${res.data.next_cursor}`);
      }
  ...
  // ✨ NEW: Load more videos when reaching end of list
  ...
  // ✨ NEW: Render loading indicator at the bottom
  const renderFooter = () => {
  ...
  // 🔹 Pull to refresh handler - MODIFIED
  const onRefresh = async () => {
    console.log("🔄 Pull to refresh triggered - Fetching latest videos...");
    setRefreshing(true);
    await fetchVideos(false); // Reset to first page
    setRefreshVersion((v) => v + 1);
    setRefreshing(false);
    console.log("✅ Refresh complete!");
  };

  // 🔹 Delete handler - MODIFIED to maintain pagination state
  const handleDelete = async (publicId, filename) => {
    console.log("🗑️ DELETE BUTTON CLICKED for:", filename);
  ...
  // ✨ MODIFIED: Sort handler - now sorts the local videos array
  const handleSortToggle = () => {
    console.log("🔄 Toggling sort order...");
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ✨ MODIFIED: FlatList with pagination */}
      <FlatList
        data={videos}
        keyExtractor={(item) => item.public_id}
        renderItem={({ item }) => {
        ...
        refreshing={refreshing}
        onRefresh={onRefresh}
        onEndReached={loadMoreVideos} // ✨ Load more when reaching end
