...
export default function StationsList() {
    ...
    const loadMore = () => {
        ...
        setTimeout(() => {
            const nextItems = allStations.slice(
                totalLoaded,
                totalLoaded + PAGE_SIZE
            );

            setDisplayedStations(prev => [...prev, ...nextItems]);
            setPage(prev => prev + 1);
    ...
    // const directionTo = (lat: number, lng: number, name: string) => {
    //     const url =
    //         Platform.OS === "ios"
    ...
    const directionTo = async (lat: number, lng: number, name: string) => {
        const url =
            Platform.OS === "ios"
                ? `http://maps.apple.com/?daddr=${lat},${lng}&q=${encodeURIComponent(name)}`
                : `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`;
        ...
        try {
            ...
            if (!canOpen) {
                Alert.alert(
                    "Route Unavailable",
                    "Unable to find a route to this location."
                );
                return;
            }
        ...
        <View key={item.id} style={styles.card}>
            ...
            <View style={styles.buttonRow}>
                <TouchableOpacity
                    ...
                    onPress={() => directionTo(item.latitude, item.longitude, item.name)}
                >
